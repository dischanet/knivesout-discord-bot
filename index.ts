...
async function fetchTwitter() {
  return await require("isomorphic-fetch")(
...
function toStrFromTweet($: CheerioStatic, elm: CheerioElement) {
  if (elm.type == "text") {
    ...
    if (
      elm.tagName == "a" &&
      elm.attribs["class"].includes("twitter-hashtag pretty-link")
    ) {
      ...
      elm.tagName == "a" &&
      elm.attribs["class"].includes("twitter-timeline-link") &&
      !elm.attribs["class"].includes("u-hidden")
    ) {
...
function getMedia($: CheerioStatic, tweetElm: CheerioElement) {
  ...
  let photoUrlProp =
    photoElement.length != 0 ? { photoUrl: photoElement.attr("src") } : {};

  // SSRではvideoタグがレンダリングされないのでかわりにサムネイルを取得する
  const videoElement = $(
    ".AdaptiveMedia-videoContainer .PlayableMedia-player",
    tweetElm
  );
  let videoUrlProp =
    videoElement.length != 0
      ? {
          photoUrl: videoElement
...
function richEmbedFromTweet(tweet: any) {
  ...
  embed
    ...
    .setAuthor(
      ...
      "https://pbs.twimg.com/profile_images/933161515602997248/ulIvWXEC_400x400.jpg"
    )
